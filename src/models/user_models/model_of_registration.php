...
class RegModel {
	...
	public function regFormSendingControl($basa) {
		try {
			if (isset($_POST['submit2'])) {
				$this->regFormFieldsEmptinessControl($basa);
			}
		}
	...
	public function	regFormFieldsEmptinessControl($basa) {
		try {
			...
			if (!empty($_REQUEST['password2']) and !empty($_REQUEST['login2']) and !empty($_REQUEST['email2']) and !empty($_REQUEST['name']) and !empty($_REQUEST['surname']) and !empty($_REQUEST['date_of_birth']) and !empty($_REQUEST['gender'])) {
				...
				$this->loginFreeControl($basa, $row, $password, $login, $email, $name, 
	...
	public function	loginFreeControl($basa, $row, $password, $login, $email, $name, 
				$surname, $date_of_birth, $gender) {
		try {
			//If $row is empty - the login is free
			if (!isset($row['login'])) {
				$this->formDataTrueValidation($basa, $row, $password, $login, $email, $name, 
				$surname, $date_of_birth, $gender);
			}
	...
	public function	formDataTrueValidation($basa, $row, $password, $login, $email, $name, 
				$surname, $date_of_birth, $gender) {
		try	{
